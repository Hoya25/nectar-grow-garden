...
-- Fix JSONB cast error in award_affiliate_nctr (5-param version)
...
BEGIN
  -- Get user's opportunity status and multiplier
  ...
  IF v_opportunity_status IS NOT NULL THEN
    ...
    WHERE status_name = v_opportunity_status;
  END IF;
  ...
  INSERT INTO nctr_transactions (
    ...
    p_user_id, v_final_amount, 'earn', p_earning_source,
  ...
  -- Check for alliance token configuration
  SELECT 
    eo.alliance_token_ratio,
    eo.alliance_token_symbol
  ...
  WHERE b.name = p_brand_name
    AND eo.alliance_token_enabled = true
  LIMIT 1;
  ...
  IF v_alliance_token_ratio > 0 AND v_alliance_token_symbol IS NOT NULL THEN
    ...
    -- Get existing alliance_tokens, safely handle NULL or non-object values
    SELECT COALESCE(alliance_tokens, '{}'::jsonb)
    INTO v_existing_tokens
    FROM nctr_portfolio
    WHERE user_id = p_user_id;

    -- If it's not a jsonb object, reset to empty object
    IF v_existing_tokens IS NULL OR jsonb_typeof(v_existing_tokens) != 'object' THEN
      v_existing_tokens := '{}'::jsonb;
    END IF;

    -- Safely extract existing token amount
    ...
    -- Update portfolio with new token balance
    ...
    WHERE user_id = p_user_id;
  ELSE
    -- No alliance token, just update NCTR
    UPDATE nctr_portfolio
    SET 
      available_nctr = available_nctr + v_final_amount,
      total_earned = total_earned + v_final_amount,
  ...
  RETURN jsonb_build_object(
    ...
    'multiplied_amount', v_final_amount,
