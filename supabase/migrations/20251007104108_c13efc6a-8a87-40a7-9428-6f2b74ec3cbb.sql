...
-- Update process_referral_reward to check email verification
...
DECLARE
  ...
  base_referrer_reward NUMERIC := 1000.00;
  base_referee_reward NUMERIC := 1000.00;
  final_referrer_reward NUMERIC;
  final_referee_reward NUMERIC := 1000.00;
...
BEGIN
  ...
  -- ⚡ NEW: Check if referred user's email is verified
  IF NOT public.is_email_verified(referral_record.referred_user_id) THEN
    RETURN jsonb_build_object(
      'success', false, 
      'error', 'Email verification required. The referred user must verify their email before rewards can be credited.'
    );
  END IF;
