-- Fix create_withdrawal_request function to correctly parse treasury_config values
...
DECLARE
  ...
  daily_limit NUMERIC;
  gas_fee NUMERIC := 0; -- NO GAS FEES - completely free withdrawals
  net_amount NUMERIC;
  treasury_enabled BOOLEAN;
  today_withdrawn NUMERIC;
  new_request_id UUID;
BEGIN
  -- Get treasury configuration (fix: cast setting_value directly, not as JSON)
  SELECT setting_value::NUMERIC INTO min_withdrawal 
  FROM treasury_config WHERE setting_key = 'min_withdrawal_amount';
  
  SELECT setting_value::NUMERIC INTO max_withdrawal 
  FROM treasury_config WHERE setting_key = 'max_withdrawal_amount';
  
  SELECT setting_value::NUMERIC INTO daily_limit 
  FROM treasury_config WHERE setting_key = 'daily_withdrawal_limit';
  
  SELECT setting_value::BOOLEAN INTO treasury_enabled 
  ...
  -- Net amount equals full withdrawal amount (no fees deducted)
  net_amount := p_nctr_amount;
  ...
  INSERT INTO nctr_transactions (
    ...
    'withdrawal',
    -p_nctr_amount,
    'NCTR withdrawal request to ' || LEFT(p_wallet_address, 10) || '... (fee-free)',
    'withdrawal',
    'pending'
  );
