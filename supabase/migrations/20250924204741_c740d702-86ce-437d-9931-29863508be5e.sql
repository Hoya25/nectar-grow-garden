-- Fix referral reward multiplier system to properly apply status multipliers
...
DECLARE
  ...
  base_referee_reward NUMERIC := 1000.00;  -- Base 1000 NCTR reward for new user
  final_referrer_reward NUMERIC;
  final_referee_reward NUMERIC;
...
BEGIN
  ...
  -- Apply reward multipliers based on user status
  SELECT public.apply_reward_multiplier(referral_record.referrer_user_id, base_referrer_reward) INTO final_referrer_reward;
  SELECT public.apply_reward_multiplier(referral_record.referred_user_id, base_referee_reward) INTO final_referee_reward;
  ...
  -- Update total_earned for referee with multiplied amount
  UPDATE public.nctr_portfolio
  ...
  SELECT public.auto_lock_earned_nctr(
    ...
    base_referrer_reward, -- Pass base amount, auto_lock will apply multiplier again
    'referral',
    'invite'
  ) INTO referrer_lock_id;
  
  -- Auto-lock referee's reward in 360LOCK (using base amount since auto_lock applies multiplier)
  SELECT public.auto_lock_earned_nctr(
    referral_record.referred_user_id,
    base_referee_reward, -- Pass base amount, auto_lock will apply multiplier again
    'referral_signup',
    'invite'
  ...
  INSERT INTO public.nctr_transactions (
    ...
    'Referral reward: ' || referee_display_name || ' joined The Garden! (' || final_referrer_reward || ' NCTR with ' || (final_referrer_reward / base_referrer_reward) || 'x multiplier in 360LOCK)',
    'referral',
    'completed'
  );
  
  -- Record referee transaction with multiplied amount
  INSERT INTO public.nctr_transactions (
    user_id,
    transaction_type,
    nctr_amount,
    ...
    'Welcome bonus: Invited by ' || referrer_display_name || ' (' || final_referee_reward || ' NCTR with ' || (final_referee_reward / base_referee_reward) || 'x multiplier in 360LOCK)',
    'referral_signup',
    'completed'
  );
  ...
  RETURN jsonb_build_object(
    ...
    'referee_multiplier', (final_referee_reward / base_referee_reward),
