-- Fix revoke_fraudulent_nctr to use 'cancelled' status instead of 'revoked'
-- The nctr_locks table check constraint doesn't allow 'revoked' as a status
...
BEGIN
  ...
  -- Mark all active locks as cancelled (not 'revoked' which violates check constraint)
