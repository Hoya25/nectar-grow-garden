-- First, update the award_affiliate_nctr function to handle alliance tokens
...
BEGIN
  ...
  IF p_brand_name IS NOT NULL AND p_purchase_amount IS NOT NULL THEN
    ...
    -- Calculate alliance token amount if enabled
    IF FOUND AND brand_data.alliance_token_ratio > 0 THEN
      alliance_token_amount := p_purchase_amount * brand_data.alliance_token_ratio;
      ...
      RAISE NOTICE '💎 Alliance Token Credit: % × % %/$ = % %',
        p_purchase_amount, alliance_token_ratio, alliance_token_symbol, 
