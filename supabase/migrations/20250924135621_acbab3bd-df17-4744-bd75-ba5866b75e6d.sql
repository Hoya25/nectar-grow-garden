-- Update daily check-in to use only admin-specified amounts
-- Remove all hardcoded legacy allocations

-- Drop and recreate the process_daily_checkin function to only use admin-specified amounts
DROP FUNCTION IF EXISTS public.process_daily_checkin(uuid);
...
BEGIN
  ...
  -- Calculate rewards based on admin configuration ONLY
  ...
  ELSIF daily_opportunity.reward_distribution_type = 'combined' THEN
    ...
    lock_90_reward := COALESCE(daily_opportunity.lock_90_nctr_reward, 0);
    lock_360_reward := COALESCE(daily_opportunity.lock_360_nctr_reward, 0);
  ELSIF daily_opportunity.reward_distribution_type = 'legacy' THEN
    -- For legacy mode, use only the nctr_reward as available NCTR
    available_reward := COALESCE(daily_opportunity.nctr_reward, 0);
  ELSE
    -- Default to available if distribution type is not set
    available_reward := COALESCE(daily_opportunity.available_nctr_reward, 0);
  END IF;
  ...
  IF total_reward <= 0 THEN
    RETURN jsonb_build_object(
      'success', false,
      'error', 'Daily check-in reward is not configured. Please contact admin.'
    );
  END IF;
