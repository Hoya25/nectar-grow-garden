...
CREATE VIEW public.user_status_details AS
SELECT 
    p.user_id,
...
ALTER VIEW public.user_status_details SET (security_invoker = true);

-- Add RLS policy to ensure users only see their own data
CREATE POLICY "Users can only view their own status details"
ON public.user_status_details
FOR SELECT
