...
const LockCommitmentModal = ({ availableNCTR, onLockCreated }: LockCommitmentModalProps) => {
  ...
  const lockOptions = [
    ...
    },
    {
      ...
      benefits: ['Qualifies for Crescendo tiers', 'Elite partner access', 'VIP experiences & events', 'Exclusive rewards program', 'Priority Alliance status'],
                    ...
                    {selectedType === '360LOCK' && (
                      <div className="mt-3 p-3 bg-primary/10 rounded-lg border border-primary/20">
                        <p className="text-sm text-primary font-medium leading-relaxed">
                          💎 360LOCK qualifies for Crescendo tiers with earning multipliers up to 2.0x!
                        </p>
                      </div>
                    )}
                  </CardContent>
